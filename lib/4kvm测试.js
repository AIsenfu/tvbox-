...
async function homeVod() {
    ...
    // 提取精选标题区域
    $("#featured-titles .owl-item").each(function() {
        const a = $(this).find("a");
        ...
        if (a.length && img.length && title.length) {
            ...
            const id = href.replace(/.*\/(movies|tvshows)\/(.*)/, "$2");
            videos.push({
                vod_id: id,
                vod_name: title.text(),
    ...
    // 提取电影推荐区域
    $("#dt-movies .owl-item").each(function() {
        const a = $(this).find("a");
        const img = $(this).find("img");
        const title = $(this).find(".data h3");
        const year = $(this).find(".data span");
    ...
    // 提取电视剧推荐区域
    $("#dt-tvshows .owl-item").each(function() {
        const a = $(this).find("a");
        const img = $(this).find("img");
        const title = $(this).find(".data h3");
        const year = $(this).find(".data span");
...
async function category(tid, pg, filter, extend) {
    ...
    // 提取视频列表
    let videos = [];
    $("article.w_item_c").each(function() {
        const a = $(this).find("a");
        const img = $(this).find("img");
        const title = $(this).find(".data h3");
        const year = $(this).find(".data span");
